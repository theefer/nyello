...
Dispatcher::Dispatcher()
	: client( Nyello::CLIENT_NAME ), readlineListener( NULL )
{

}

Dispatcher::~Dispatcher() {
	// readlineListener is auto-destroyed by mainloop
}
...
{
	try {
		char* cpath = std::getenv("XMMS_PATH");
		client.connect( cpath ? cpath : "" );
	}
	catch( connection_error& e ) {
		// FIXME: failed to connect, try to start the server and reconnect (if fails, quit)
		throw;
	}
}
...
void
Dispatcher::execute( const string& input )
{
	// FIXME: Run the command
	cout << "run command: " << input << endl;
}
...
Dispatcher::loop()
{
	try {
		...
		readlineListener = ReadlineListener::getInstance( *this );
		client.getMainLoop();
		MainLoop& ml = dynamic_cast<MainLoop&>( client.getMainLoop() );
		ml.addListener( readlineListener );
...
Dispatcher::stop()
{
	// We destroy the client to disconnect
	delete this;
}

const string
Dispatcher::getPrompt() const
{
	// FIXME: dynamic prompt
	return string("nyello> ");
}
